...
def BUTTON(screen, position, text):
    ...
    font = pygame.font.Font('./resources/font/PingPong.ttf', 50)
...
def start_interface(screen):
    ...
    while True:
        button_1 = BUTTON(screen, (330, 190), 'One Player')
        button_2 = BUTTON(screen, (330, 305), 'Two Player')
...
def GameDemo(num_player, screen):
    ...
    # Группа спрайтов игрока, пули и астероида
    playerGroup = pygame.sprite.Group()
    bulletGroup = pygame.sprite.Group()
    asteroidGroup = pygame.sprite.Group()
    # Временной интервал создания астероида
    asteroid_ticks = 90
    ...
    while True:
        ...
        if (Score_1 + Score_2) < 500:
            ...
            background = bg_3
        # Чтоб фон двигался, чтоб добиться визуального полета корабля
        ...
        # Отрисовка счета
        Score_1_text = 'Score-One Pleer: %s' % Score_1
        Score_2_text = 'Score-Two Pleer: %s' % Score_2
...
#Реализация game_over  На последнем интерфейсе

def end_interface(screen):
    clock = pygame.time.Clock()
    while True:
        button_1 = BUTTON(screen, (330, 190), ' Restart')
        button_2 = BUTTON(screen, (330, 305), '  Exit')
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
...
def main():
    ...
    pygame.display.set_caption('AstroWar (Duo)')
